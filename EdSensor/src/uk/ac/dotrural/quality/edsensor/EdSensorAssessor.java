...
public class EdSensorAssessor {
	...
	private final String ENDPOINT = "http://dtp-126.sncs.abdn.ac.uk:8080/openrdf-sesame/repositories/CarWalkTrain";
	...
	public ArrayList<Observation> getObservations()
	{
		...
		String query = "SELECT * WHERE {" + 
					   "    ?obs a <http://purl.oclc.org/NET/ssnx/ssn#Observation> . " +
					   "	?obs <http://purl.oclc.org/NET/ssnx/ssn#featureOfInterest> ?foi . " +
					   "	?obs <http://purl.oclc.org/NET/ssnx/ssn#observedProperty> ?prop . " +
					   "	?obs <http://purl.oclc.org/NET/ssnx/ssn#observedBy> ?sens . " +
					   "	?obs <http://purl.oclc.org/NET/ssnx/ssn#observationResultTime> ?rTime . " +
					   "	?obs <http://purl.oclc.org/NET/ssnx/ssn#observationServerTime> ?sTime . " +
					   "	?obs <http://dtp-126.sncs.abdn.ac.uk/quality/CarSensor/Event> ?event . " +
					   
					   "	?obs <http://purl.oclc.org/NET/ssnx/ssn#observationResult> ?res . " +
					   "	?res <http://purl.oclc.org/NET/ssnx/ssn#hasValue> ?val . " +
					   
					   "	OPTIONAL {" +
					   "		?val <http://purl.oclc.org/NET/ssnx/ssn#hasValue> ?value . " +
					   "	}" +
					   
					   "	OPTIONAL {" +
					   "		?val <http://purl.oclc.org/NET/ssnx/ssn#hasValue> ?value . " +
					   "		?val <http://dtp-126.sncs.abdn.ac.uk/quality/CarSensor/AccelerationDirection> ?dir . " + 
					   "	}" +
					   
					   "	OPTIONAL {" +
					   "		?val <http://purl.oclc.org/NET/ssnx/ssn#hasValue> ?value . " +
					   "		?val <" + NS + "satellites> ?sats . " +
					   "	}" +
					   
					   "	OPTIONAL {" +
					   "		?val <http://www.w3.org/2003/01/geo/wgs84_pos#lat> ?lat . " +
					   "		?val <http://www.w3.org/2003/01/geo/wgs84_pos#long> ?lon . " +
					   "		?val <" + NS + "satellites> ?sats . " +
					   "		?val <" + NS + "precision> ?prec . " +
					   "	}" +
		...
		try {
			...
			while(rs.hasNext())
			{
				...
				Literal value;
				Literal sats;
				switch(ObservationType.strToObsType(prop.getLocalName()))
				{
				case ACCELERATION:
					...
					AccelerometerObservation aObs = new AccelerometerObservation(
											observation.getURI(),
											ObservationType.strToObsType(prop.getLocalName()),
											foi.getURI(),
											sens.getURI(),
											res.getURI(),
											val.getURI(),
